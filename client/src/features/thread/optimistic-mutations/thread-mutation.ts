 ...
 * @returns 
 */
export const createReactionMutationConfig = (queryClient: QueryClient, userId: string, thread: Thread) => ({
  onMutate: async (newReaction: CreateReactionType) => {
    const userReactionQKey = getUserReactionQueryOptions({ userId, threadId: thread.id }).queryKey;
    ...
    queryClient.setQueryData(userReactionQKey, (currentReaction: { type: ReactionType } | undefined) => {
      if (!currentReaction) return currentReaction;
      return {
        type: userId.toString() === newReaction.userId.toString()
          ? newReaction.type
          : currentReaction.type,
      };
    });

    queryClient.setQueryData(threadReactionsQKey, (currentThread: Thread | undefined) => {
      if (!currentThread) return currentThread;
      return {
        ...currentThread,
        likeCount: newReaction.type === "LIKE" ? currentThread.likeCount + 1 : currentThread.likeCount,
        dislikeCount: newReaction.type === "DISLIKE" ? currentThread.dislikeCount + 1 : currentThread.dislikeCount,
      };
    });

    queryClient.setQueryData(generalThreadsQKey, (currentThreads: Thread[] | undefined) => {
 ...
 * @returns 
 */
export const saveThreadMutationConfig = (queryClient: QueryClient, thread: Thread) => ({
  onMutate: async (saveThread: { threadId: string }) => {
    ...
    if (thread.communityId) {
      const communityThreadsQKey = getThreadsByCommunityQueryOptions(thread.communityId).queryKey;
      
      queryClient.setQueryData(communityThreadsQKey, (currentThreads: Thread[] | undefined) => {
        if (!currentThreads) return currentThreads;
        return currentThreads.map((currentThread) => {
          if (currentThread.id === saveThread.threadId) {
            return { ...currentThread, isSaved: true };
