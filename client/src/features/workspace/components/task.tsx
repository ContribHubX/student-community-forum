...
export const Task = ({ task }: TaskProp) => {
  ...
  const { text, background } = statusColors[task.status === "todo" ? "archived" : task.status === "doing" ? "finished" : "active"];
  ...
  return (
    ...
    >
        ...
        >
            ...
            <p className="text-[.8rem] text-muted-foreground font-light">{task.description}</p>
        </FlexContainer>
