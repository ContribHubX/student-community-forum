...
export const SelectRequest = ({ questionId, currentUserId }: SelectRequestProp) => {
  ...
  return (
    <div>
        <div className="relative"> 
            <IoIosSearch 
                className="text-xl text-muted-foreground mt-3 left-3 absolute"
            />
            <input 
                type="text" 
        ...
        <div
            className="min-h-[300px] flex flex-col gap-3 mt-6"
        >
            {users?.map(user => (
                <UserItem 
                    key={user.id}
