...
import {
  ...
  IQuestionRequest,
  IQuestionCreation,
} from "@/domain/interfaces/IQuestion";
...
class QuestionService {
   ...
   */
  public async createQuestionRequest(
    dto: IQuestionRequestDto,
  ): Promise<IQuestionRequest> {
    try {
      const result = await this.questionRepo.request(dto);

      this.eventManager.publishToOne<IQuestionRequest>(
        "request--new",
        result,
        result.requestedTo.id,
      );
   ...
   */
  public async getQuestionsRequestedTo(
